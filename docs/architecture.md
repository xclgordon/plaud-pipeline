...
一个**全自动化管线**，将会议录音经过云端同步 → 说话人分割 → 身份具名 → AI 纪要合并 → Wiki 知识库生成，最终呈现为 Notion 风格 Web 知识库。
...
| 优先级 | 来源 | 置信度 | 方法 |
|--------|------|--------|------|
| 1 | Plaud 标签匹配 | ~100% | 时间戳对齐 Plaud 的具名标注 → spk_id |
| 2 | 声纹传播 | >0.55 cos | CAM++ embedding + 余弦相似度匹配已知声纹库 |
| 3 | LLM 推断 | 中 | LLM + 组织架构图 + 共现关系 + 称呼惯例 |
| 4 | 兜底 | — | spk_N |

### 3. 先 Plaud、后 AI（Plaud-first Strategy）

CAM++ 声纹跨录音不可靠（同一人在不同录音的余弦相似度 0.50–0.65，反而不如不同人在同一录音 0.85–0.97）。**Plaud 标签命中了 95%，声纹传播填补剩余，LLM 处理最后边缘情况。**
...
```
    ...
    ┌────▼─────┐    ┌───────▼────────┐    ┌───────▼────────┐
    │ ① Cloud  │    │ ② Diarize       │    │ ③ Name         │
    │  Sync    │───▶│ pyannote +      │───▶│ 3-Phase        │
    │          │    │ FunASR (GPU)    │    │ Anchor Naming  │
    └──────────┘    └──────────────────┘    └───────┬────────┘
                                                    │
    ┌────▼──────┐    ┌────────────────┐    ┌───────▼────────┐
    │ ④ AI Notes│───▶│ ⑤ Notify       │───▶│ ⑥ Wiki Rebuild │
    │   Merge   │    │ (Telegram)     │    │ (only when new)│
    └──────────┘    └────────────────┘    └───────┬────────┘
                                                  │
                                       ┌──────────▼──────────┐
                                       │ Wiki Server (8899)  │
...
### Phase 1: Plaud 标签匹配（95% 命中率）

通过时间戳将 Plaud 的具名 block 对齐到 pyannote 的 spk_id，直接建立映射。**地面真相，100% 可靠。**
...
Claude + 组织架构图 + 共现关系 + 中文称呼惯例（"X总"=姓氏+总，"老X"=熟悉称呼），cap=8 候选。
...
| 层次 | 技术 |
|------|------|
| 录音 | Plaud NotePin / 任意 OGG 文件 |
| 分割 | pyannote community-1 (GPU) |
| ASR | FunASR Paraformer-large |
| 声纹 | FunASR CAM++ 192-dim |
| LLM | Claude Sonnet / OpenAI 兼容接口 |
...
每层只做自己擅长的事：Plaud 标签→确定性匹配，声纹→概率匹配，LLM→语义兜底。

### 3. 中文搜索需要 Bigram

无空格中文："广东华为" → ["广东","东华","华为"]，无需引入 jieba。

### 4. S3 预签名 URL 不能带 Auth Header

`Authorization: Bearer` + `X-Amz-Algorithm` → `Only one auth mechanism allowed`。必须用干净 session。

### 5. LLM JSON 多策略解析

Code fence → bare regex → trailing comma cleanup。单一策略会静默丢弃结果。

### 6. pyannote 对 OGG 敏感

先 ffmpeg 转 16kHz WAV 最稳妥。

### 7. GPU 显存管理

54 分钟音频不带 VAD → 43GB → OOM。**始终启用 fsmn-vad。**
